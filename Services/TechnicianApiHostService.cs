...
public sealed class TechnicianApiHostService : IDisposable
{
            ...
            app.Use(async (ctx, next) =>
            {
                ...
                if (isTechApi)
                {
                    ...
                    if (!IsApiKeyValid(providedKey, _apiKey))
                    {
                        ...
                        await ctx.Response.WriteAsJsonAsync(new { ok = false, message = "Unauthorized. Invalid technician API key." });
                        return;
                    }
                }
    ...
    private static void MapApiEndpoints(IEndpointRouteBuilder app)
    {
        ...
        app.MapPost("/api/tech/job-cards/{jobCardId:int}/photos", async (HttpRequest request, int jobCardId) =>
        {
            ...
            try
            {
                ...
                return Results.Ok(new
                {
                    ...
                    message = "Photo uploaded to job card."
                });
            }
            finally
            {
