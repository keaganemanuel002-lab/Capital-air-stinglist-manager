    ...
    public byte[] BuildQuotePdf(Quote quote)
    {
        ...
        return Document.Create(container =>
        {
            ...
            page.Content().Column(col =>
            {
                ...
                // Optional logo
                var logoPath = Path.Combine(AppContext.BaseDirectory, "Assets", "capital-air-logo.png");
                ...
                col.Item().Table(table =>
                {
                    ...
                    table.Header(header =>
                    {
                        ...
                        header.Cell().Background(Colors.Grey.Lighten3).Padding(6).AlignRight().Text("Qty").Bold();
                        header.Cell().Background(Colors.Grey.Lighten3).Padding(6).AlignRight().Text("Unit Price").Bold();
                        header.Cell().Background(Colors.Grey.Lighten3).Padding(6).AlignRight().Text("Amount").Bold();
                    });

                    foreach (var item in quote.LineItems ?? Enumerable.Empty<QuoteLineItem>())
                    {
                        ...
                        var unitPrice = item.UnitPriceExVat > 0
                            ? item.UnitPriceExVat
                            : (item.LineTotalExVat / qty);

                        table.Cell().Border(1).Padding(6).Text(description);
                        table.Cell().Border(1).Padding(6).AlignRight().Text(qty.ToString());
                        table.Cell().Border(1).Padding(6).AlignRight().Text($"R {unitPrice:0.00}");
                        table.Cell().Border(1).Padding(6).AlignRight().Text($"R {item.LineTotalExVat:0.00}");
                    }
                });
